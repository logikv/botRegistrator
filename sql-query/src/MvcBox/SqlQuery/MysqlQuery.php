...
class MysqlQuery extends SqlQueryAbstract
{
    /**
     * @return array
     */
    public function queryTemplates()
    {
        return array(
            ...
            self::QUERY_TYPE_UPDATE => 'UPDATE {{table_real}} SET {{update}} {%where%}WHERE {{where}}{%/where%} {%order_by%}ORDER BY {{order_by}}{%/order_by%} {%limit_count%}LIMIT {{limit_count}}{%/limit_count%}',
            self::QUERY_TYPE_DELETE => 'DELETE FROM {{table_real}} {%where%}WHERE {{where}}{%/where%} {%order_by%}ORDER BY {{order_by}}{%/order_by%} {%limit_count%}LIMIT {{limit_count}}{%/limit_count%}',
